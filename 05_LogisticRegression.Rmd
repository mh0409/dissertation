...
```{r}
vars_of_interest <- c("country", "gor_dv", "urban_dv", "racel_recat", "sex_dv", "jbstat_recat", "marstat_recat", "fihhmnnet1_dv", "houscost1_dv", "is_mtgprisoner") #, "hhsize") # to do once i re-download all data w/ "hhsize"
...
# train_set$marstat <- fct_drop(train_set$marstat) ## not using these vars anymore -- replaced with marstat_recat
...
for (i in 1:10) {
  ...
  mod <- glm(is_mtgprisoner ~ gor_dv + urban_dv + racel_recat + sex_dv + jbstat_recat + marstat_recat + fihhmnnet1_dv + houscost1_dv, data = train_set, family = binomial)
...
predicted_classes <- ifelse(probabilities > 0.5, "mtg_prisoner", "not_mtg_prisoner")
